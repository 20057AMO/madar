...
| Project team & access | `tests/team-access.test.ts` | Real Docker: owner creates project → creates editor+viewer users → adds members → permission matrix (viewer read-only/403s, editor read+write+stop/start, member add denied, **viewer duplicate 403**), remove rules, outsider 403, **global-editor matrix** (non-member editor reads/writes any project 200, but add-member/transfer-owner/delete stay 403), **sudo transfer contract** (missing password 400 → wrong password 401; the 403 gate opens for owner/system-admin/**admin members** and closes for others; success path real-login self-skips and asserts deterministic ownership: new owner + former owner both explicit admin members), **self-transfer 400**, **owner-delete guard** (refuses 400 while the user owns projects), then deletes project + temp users |
...
| Opencode Studio | `tests/opencode-studio.test.ts` | Full baked roster (28 agents / 20 skills / 8 slash commands), agent+skill+command CRUD lifecycles, kebab-name + traversal rejection, frontmatter-required command writes, **roster integrity check** (every baked file: frontmatter present, zero NUL bytes, description non-empty — regression guard after a skill once shipped as pure NULs), config merge preserves keys / rejects junk, version-info shape (against running container) |
| Agent delegation (core) | `tests/opencode-delegate-core.test.ts` | 13 offline units on the import-free `opencode-delegate-core.ts` (like serve-core/janitor-core): `agentCapability` safe-by-default classification (`edit: deny\|false\|no` under `permission:` → **readonly** — including the flow form `permission: {edit: deny}`; a bare top-level `edit: deny`, a `description` that merely mentions it, a typo key or an upper-case `DENY` → **write**, so a permission-less agent can never sneak past the editor gate), `sanitizeDelegatePrompt` (trim; empty/null/junk → 400 `HttpError`; 20 000-char truncation, not drop), `buildDelegatePrompt` (forces `/workspace/WSD_PROJECT.md` + `WSD_CANVAS.md` first, embeds the task, per-agent keyword template review/test/debug/plan/generic, slug fallback when the name is missing), `delegateEntryId` (`d-<6>-<6>` uniqueness across a batch), `capDelegations` (keeps the newest, junk/null-tolerant), `validAgentName` (kebab-case roster names only — path/space/junk rejected) |
...
| Project activity feed | `tests/activity-core.test.ts` (19 offline units on the import-free `activity-core.ts`: the **34-event vocabulary**, whitelist rejection, `sanitizeDetails` field/cap rules, append/cap ceiling, legacy backfill + kebab map, newest-first paging, entryId format) + `tests/project-activity.test.ts` (real Docker: `created`/`started`/`stopped` attribution, config/content/team/snapshots/ports/limits events with details, **project list & detail carry the same feed** — `lastTouched`-readable append order, pagination + honest `total`, access matrix 401/403/200/200, 404 unknown + 404 after delete, per-project cleanup) |
...
| WebSocket matrix | `tests/websocket.test.ts` | 6 endpoints × {no token→401, valid→open, invalid→401}; every describe creates its OWN dedicated throwaway probe project (`ws-*`, torn down in `after`) — real user projects are NEVER reused (a reused project once absorbed `member_added`/`member_removed` activity entries that live forever, plus uploads/files) |
| Team chat (V1) | `tests/chat-team-core.test.ts` (23 offline) + `tests/chat-team-api.test.ts` (14 real-Docker): channel id discipline/sort/labels, message sanitize/normalize/cap-prune/search/pin, **suite self-cleans its channels in `after()`** + **deleting a user cascade-removes their direct channels** (`removeUserChannels` in chat-team-store) so accumulated test DMs never leak onto a shared server; API: 401 matrix, manual create 201/dup 409/junk 400, traversal-safe channel ids, message send+@mentions, replyTo dangling-ref drop, case-insensitive channel-scoped search, pin roundtrip, unread via read positions, delete by admin+creator only + kind guard, idempotent direct DM + self-chat 400, project auto-channel created-with/deleted-with project, access matrix (project channel mirrors membership; manual team-wide), attachment upload/authed-download/oversize 400, presence shape |
...
| `/project/:slug` | Project | Detail: overview, AI chat, files, logs, **Terminal** (per-project shell, the SAME `ProjectTerminal` component — tabs/project+control modes/history/zoom/reconnect/quick commands), notes, scripts, **canvas** (visual planning whiteboard), **Agents** (`AgentRunPanel` — run a roster subagent on the workspace, live tail + per-project History, land results into Notes/Canvas) |
...
### Project activity feed (من قام بماذا ومتى)
- Per-project action history stored in `data/projects/<slug>/activity.json` (mode 0600, oldest→newest, capped at **200** entries) answering "من قام بماذا ومتى" — the roadmap's team-collaboration item, backend-only (feed read via the API; the frontend already renders `project.activity`-derived hints and `lib/time.ts`'s `lastTouched`)
- **`services/activity-core.ts`** (pure, import-free like janitor-core): the canonical **34-event vocabulary** (`ACTIVITY_ACTIONS` — lifecycle: created/recreated/duplicated/imported/restored/deleted/started/stopped/cloned · config: updated/env_updated/tags_updated/ports_updated/limits_updated · content: notes_saved/canvas_saved · team: member_added/member_removed/member_role_changed/ownership_transferred · snapshots: snapshot_captured/snapshot_deleted/snapshot_config/exported · serve: serve_started/serve_stopped · crash: crashed/crash_cleared · file reviews: review_opened/review_commented/review_resolved/review_reopened/review_deleted · agent delegation: agent_run) plus `isActivityAction`, `sanitizeDetails` (numbers/strings/string-arrays only, ≤400 serialized chars — booleans/objects/nested dropped), `appendActivity`/`capActivity` (injectable ceiling), `sliceActivity` (newest-first + honest total), `backfillFromLegacy` (kebab→snake `LEGACY_ACTION_MAP`, unknown skipped, order preserved) and `entryId()` (`a-<6>-<6>` base36)
- **`services/project-activity.ts`** store: `recordActivity(slug, action, {userId, actorName, details, at})` writes under a per-slug `withFileLock('activity:<slug>')` sync lock, **never throws** (a failed feed write must not break a lifecycle op), and resolves `actorName` via `getUserInfo` — **no actor → `'System'`** (crash detector, `ensureServeRunning`, scheduled snapshot sweep). `loadActivity(slug)` sanitizes rows and **lazily backfills legacy `meta.activity`** into activity.json on first read (meta.json read directly via fs — no projects-meta import, avoiding a cycle; best-effort persist so the migration runs once). `listActivity(slug, {limit, offset})` reverses newest-first + `total` and enriches `actorDisplayName`/`actorAvatarExt` from a single `listUsers()` call
- **API**: `GET /api/projects/:slug/activity` (viewer+, `limit` ≤100 default 50, `offset` ≥0) → `{entries, total}`; 404 for unknown slugs/deleted projects. The project **list/detail carry the same feed** (`project.activity = loadActivity(slug)` in docker-manager) so existing `activity[activity.length-1].at` consumers (`lib/time.ts` lastTouched, team-page assertions) keep working — **the LIST payload is deliberately tail-only** (`{action, at}` of the newest event, no `userId`/`actorName`/`details`), because `/api/projects` is readable by ANY authenticated user while `/activity` is viewer-gated; the DETAIL payload keeps the full feed (viewer+)
- **Feed is an intentional historical record**: `member_added`/`member_role_changed`/`member_removed`/`ownership_transferred` retain `username` + `targetUserId` in their details **forever, even after the member is removed or the account deleted** — that persistence is the whole point of "من قام بماذا ومتى" (who did what, when), so a removed member's contribution stays attributable and auditable. The UI renders a removed actor as-is (stored `actorName` wins; a legacy entry whose user is gone resolves to `(deleted user)`, never the `System`/Bot icon)
- **Wired write points** (all edits via `recordActivity`, never meta writes): `createProject` (`created` — also deletes any `savedMeta.activity` so meta never re-carries the feed), `recreateProject` (`recreated` on top of `created`), `duplicateProject` (`duplicated`), `cloneIntoWorkspace` (`cloned`), `startProject`/`stopProject`, `removeProject(slug, userId?)` (`deleted` **before** `deleteMeta` — the entry lives as long as the project does), `updateProjectPorts(slug, ports, userId?)` (`ports_updated {ports:[String]}`), `updateProjectLimits(slug, patch, userId?)` (`limits_updated` = merged limits), `project-alerts` (`crashed {reason, exitCode}` System event + `manualClearCrash(slug, userId?)` → `crash_cleared`), `project-serve` (`serve_started`/`serve_stopped {port}`; `ensureServeRunning`/sweep paths record as System), `captureSnapshot(slug, userId?)` (`snapshot_captured {file, size}`), `archive-manager` restore (`restored`), index routes: PATCH→`updated`, env→`env_updated {count}`, tags→`tags_updated {tags}`, notes→`notes_saved {count}`, canvas→`canvas_saved {nodes, edges}`, members→`member_added`/`member_role_changed`/`member_removed` with `{targetUserId, username, role}`, transfer-owner→`ownership_transferred {targetUserId, username, previousOwnerId}`, snapshots config/capture/delete→`snapshot_config {intervalMin, keep}`/`snapshot_captured`/`snapshot_deleted {file}`, export/import/restore→`exported`/`imported`/`restored`, `opencode-delegate`→`agent_run` (`{agent, status:'started'}` at launch, `{agent, ok, durationMs}` on finish). `meta.activity` remains on `ProjectMeta` **only** as the legacy read/backfill source (`touchActivity` is gone)
- Coverage: `tests/activity-core.test.ts` (19 offline units — vocabulary/whitelist/cap/migration/page format) + `tests/project-activity.test.ts` (live-container: created/started/stopped attribution, config/content/team/snapshots/ports/limits events with details, list-carried feed, pagination, 401/403/404 matrix, per-project cleanup)
...
- Admin-only (`requireAdmin`) CRUD on `data/webhooks.json` (0600): `GET/POST /api/webhooks` (201), `PUT/DELETE /api/webhooks/:id`, `POST /api/webhooks/test` (saved id **or** raw url, strict rate-limited, `{ok,status,error}`). Max 50 webhooks, name/url required, URL SSRF-guarded through the same `assertFetchableHost` as providers (`ftp:`/cloud-metadata rejected, localhost+host.docker.internal fine). `events` whitelist = `crash | created | started | stopped | recreated | deleted | snapshot-saved | update-started | update-ok | update-failed | update-rolled-back | update-rollback-failed`; junk entries dropped, fully-junk falls back to `['crash']`
...
- **Hooks wired in**: `docker-manager` fires `created`/`started`/`stopped`/`deleted`/`recreated`; `project-alerts` fires `crash`; `project-snapshots-auto` fires `snapshot-saved` (capture-now/scheduled); component updates fire `update-started`/`update-ok`/`update-failed`/`update-rolled-back`/`update-rollback-failed` via `dispatchUpdateEvent` (flat payload `event`/`component`/`phase`/`at`/`from`/`to`/`error`) — `component-updates.applyCodeServer` (start+terminal verdicts) and the opencode audit-callback mirror in `applyOpencode` cover BOTH the HTTP path and OpencodeStudio's direct `applyUpdates`, and `updates-boot` emits `update-failed` (prefix `boot reapply`) when a post-rebuild re-apply fails. UX: Settings → Notifications & Webhooks panel (add row, per-webhook name/url/event checkboxes/enabled toggle, "Test" with live receiver feedback, new-secret or remove-secret input, danger-confirm delete)
...
- **Latest-at-build baseline**: the Dockerfile installs the newest versions resolved at build time — code-server via the GitHub releases API (tag regex-guarded, **SHA-256 digest verified** before install, fallback `4.96.4`) and opencode via `npm …@latest` gated to **major 1** (mirroring `SUPPORTED_MAJORS=[1]`, fallback `1.18.22`). Rebuild with `--no-cache` to re-resolve latest; `--build-arg CODE_SERVER_VERSION=<ver>` pins a reproducible code-server. Repo-root `opencode.json` (copied to `/root/.config/opencode/`) carries `$schema`, provider config, `subagent_depth: 2` and `default_agent: orchestrator` (the primary orchestrator agent)
- **Baked roster (v3 — full SDLC coverage, English prompts, trigger-engineered descriptions)**: 28 subagents in `opencode/agents/*.md` — orchestration: orchestrator (primary — plans, routes and delegates across the full roster, skills and slash commands; `mode: all`, edit/bash/task allow, does small direct fixes itself); planning/design: architect · ux-designer; implementation: frontend-developer · backend-developer · db-expert · websocket-engineer · data-engineer; quality: code-reviewer · test-writer · refactorer · debugger · perf-optimizer · accessibility-auditor (read-only); security: security-auditor (read-only) + pentester (active, bash-only with hard limits); ops/reliability: devops-engineer · release-manager · incident-responder (bash-only, edit-denied) · log-analyst · observability-engineer; APIs/docs: api-designer · doc-writer; platform: madar-expert; languages: python-expert · golang-expert · rust-expert; LLM advisory: prompt-engineer. Reviewers are `edit: deny`+`bash: deny`, implementers `allow/allow`, pentester/incident-responder bash-only. Every file follows the authoring template: `Use when…`/`Use PROACTIVELY when…` in description (MISSING_TRIGGER-style), `## When invoked`, one Input→Output few-shot example, `## Handoffs` cross-references (security-auditor→pentester, architect→implementers, reviewer→debugger…), closing principle line
- **20 skills** in `opencode/skills/<name>/SKILL.md`: clean-code, docker-debug, git-release, wsd-workflow (house rules) + planning-methodology, debugging-methodology, testing-strategy, security-hardening, api-design-guidelines, performance-profiling (methodology) + frontend-implementation, backend-implementation, database-design, websocket-implementation, code-review, refactoring, accessibility, git-workflow, documentation, typescript-practices (programmer's toolkit)
...
- **Project-page delegation (run a roster subagent on the workspace)**: `services/opencode-delegate.ts` orchestration + `opencode-delegate-core.ts` pure rules let a project member launch a named subagent from the project's **Agents** tab. API: `POST /api/opencode/delegate/:slug` (per-user write limiter) → `201 {id, status:'running'}` and the run continues in the background; `GET /api/opencode/delegate/:slug` (viewer+) → live state (`running` with a 30-line tail, or `idle`); `GET …/delegate/:slug/history` (viewer+) → `{entries, total}` newest-first; `GET …/history/:id` (viewer+) → one entry; `DELETE …/history/:id` (editor+) → `{ok:true}`. **Access is capability-gated INSIDE the service, not the router**: `agentCapability()` reads the agent's LIVE frontmatter and only an explicit `edit: deny|false|no` under `permission:` classifies **readonly** (viewer+); every other agent is a **writer** (editor+) so a permission-less agent can never sneak past the gate. Singleflight per slug (**409**) + global `MAX_CONCURRENT=2` (**429**); opencode unreachable → **503 `opencode_offline`**; forced timeout `WSD_DELEGATE_TIMEOUT_MS` (default 5 min, min 10 s) via an `AbortController` wired into every facade fetch.
- **Transport is the session path, never `--agent`**: the run creates a fresh opencode session on the project dir (`v1CreateSessionId`), switches the agent with `POST /api/session/<id>/agent` (agent/model in the prompt body are **silently ignored** by opencode, so the switch is mandatory), enqueues `POST /api/session/<id>/prompt`, then polls `/api/session/<id>/history` until a `step.ended` event. Composite numbered event names (`session.next.step.ended.2`, `session.next.text.ended.1`) are normalized back to the abstract `step.started`/`text.ended`/`step.ended` vocabulary; a drain that never emits `step.started` within **60 s** returns `started:false`, turned into the honest "opencode did not start (drain stalled)" failure instead of burning the whole timeout. The `cliDelegate` rescue (`opencode run … --session … --attach … --dir … --format json`) is restricted to editor+ **write** agents behind an explicit `allowCliFallback` flag, fires ONLY when the prompt was never admitted (never a double-run), and deliberately **never passes `--agent`** — that flag bypasses the agent's frontmatter permission deny (the upstream quirk above), so a rescue can't escalate a readonly agent.
- **Prompt + history + feed**: `buildDelegatePrompt` wraps the user task with a mandatory preparation step (read `/workspace/WSD_PROJECT.md`, then `WSD_CANVAS.md` when present, then inspect the layout) plus a per-agent working-mode template (review/test/debug/plan/generic by name keyword); prompts are sanitized + capped at 20 000 chars. History persists at `data/projects/<slug>/delegations.json` (0600, per-slug lock `delegate:<slug>`, cap 20 oldest→newest); `reconcileRunningDelegations()` flips rows orphaned in `running` by a restart to `failed` ("Interrupted by server restart") at boot. Each run appends the **`agent_run`** activity event (`{agent, status:'started'}` → `{agent, ok, durationMs}`) and audits `agent-run`/`agent-run-failed`.
- **Agent-run UI (`AgentRunPanel`)**: live roster from Opencode Studio with per-agent capability badges (`read-only`/`writer`), a template-seeded task editor (Ctrl+Enter, 20 k counter), 1.5 s status polling with a live tail, offline degradation (explains opencode is unreachable + links to `/opencode`), per-project History with delete, and — for editors on a `done` run — **Save to Notes** / **Aggregate to Canvas** reusing the existing Notes/Canvas PUT routes (`frontend/src/lib/delegate-plans.ts` parses Markdown headings/bullets into notes or sticky nodes). Viewers get the panel read-only: readonly agents only, no land-into-project actions.
...
- **الدردشة الجماعية للعمل (Team Chat)** — ✅ *مُنجزة (V1, 2026-09)*: صفحة مستقلة `/chat` — قنوات جماعية (editor+ تنشئ، المنشئ/admin يحذف) + محادثات مباشرة `dm:<a>:<b>` (تلقائية وتتماثل) + **قناة تلقائية `project:<slug>` لكل مشروع** (تُنشأ معه وتموت بحذفه)، تمرير فوري عبر `/ws/chat-team` (subscribe/unsubscribe/typing/read/pin/presence)، رسائل ≤500 لكل قناة (أقدمها تُقلم)، وسم @ وردود (replyTo) ورسائل مثبتة، بحث حساس للصيغة داخل القناة، مرفقات ≤10MB×5 (حفظ عبر بايتات موثّقة + تنزيل مُصادَق؛ البيانات خارج الاستجابة)، مؤشر حضور عبر WS (يزال مع حذف المستخدم) + `GET /api/chat-team/presence`، unread لكل عضو عبر `read.json`، صلاحيات: منظومة editor/global-editor/members تصل لكل القنوات، viewer يقرأ فقط بالقنوات الجماعية/المشروع، 401/404/400 لكل الحدود. مخزن على القرص `data/chat-team/` بقفل ملفات؛ تقنيات id محدّدة لمسارات fs-غير قابلة للتلاعب.
- **حساب المستخدم (User Account)** — ✅ *مُنجزة (2026-09)*: كل مستخدم له بروفايل في `Settings → Profile` (اسم معروض + بريد + سيرة + **صورة**، مع صف userame للقراءة فقط) **وصحفة بروفايل قابلة للعرض للغير** في `/user/:id` (تظهر الاسم/الشارة/البريد/السيرة/تاريخ الانضمام لأي عضو مصادق) تُفتح بالنقر من Team وTeamPanel والمحادثة (Team Chat: رصة حضور بأفورة قابلة للنقر + أفورة الرسائل). خصوصية بريد: toggle "Show email to team members" (`meta` في `users.json` → `profile.emailVisible`) — البريد يُفلتر خادمياً (`GET /api/users/:userId/profile`) عند غير صاحبه إذا كان مخفيًا مع بقاء العَلَمة لتفسير "Hidden" في الواجهة؛ `/me` يحتفظ به دائمًا. مغطاة في `tests/user-profile.test.ts` (دورة كاملة: يرى المالك بريده، الغير لا يراه، الروستر الإداري يحتفظ به، 400 للقيم الخاطئة، null يمسح العَلَمة)
- **تحسين إدارة الفريق (Team Management)** — ✅ *مُنجزة (2026-09)*: نقل الملكية أصبح **عملية sudo حتمية** (`ReAuthModal` + `accountPassword` + `authLimiter` + audit `ownership-transferred`/`-failed`) — مفتاح فعلي (لا حلول بديلة) يعتمد على **تملك الفعل** (owner أو عضو admin في المشروع أو نظام admin؛ عضو editor/viewer ما زال 403) مع منع النقل للذات (400). **ملكية حتمية**: المالك القديم يبقى عضو `admin` صريحًا في `meta.members` (لا فقدان صامت للوصول في المشاريع legacy)، والملك الجديد يُزامَن تلقائيًا في قناة المشروع (`ensureProjectChannel` + `setChannelMemberRole`). **حارس حذف المالك**: حذف مستخدم يملك مشاريع → 400 بأسماء المشاريع (مَنعًا للمشاريع اليتيمة بلا مالك). **UI (`TeamPanel`)**: أيقونات الإدارة (إضافة/إزالة/تغيير دور/نقل) مقيدة بـ `canManage` بدل عرض أزرار تفشل دائمًا بـ 403؛ شارة ثابتة "Owner · Admin" بدل قائمة دور مزيفة؛ تحديث فوري للشارة بعد النقل عبر `ownerId` محلي؛ إصلاح بقاء نافذة النقل المفتوحة (stale modal) عند التنقل بين المشاريع بإعادة تصفير الأهداف عند تغيّر `slug`. صفحة `Team` تُرتّب المستخدمين هرميًا (admin → editor → viewer). المغطاة في `tests/team-access.test.ts` (سودو 400/401/403، باب admin-member، self-transfer، guard الحذف) + `tests/team-page.test.ts`
