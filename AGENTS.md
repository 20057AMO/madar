...
- **`services/activity-core.ts`** (pure, import-free like janitor-core): the canonical **28-event vocabulary** (`ACTIVITY_ACTIONS` — lifecycle: created/recreated/duplicated/imported/restored/deleted/started/stopped/cloned · config: updated/env_updated/tags_updated/ports_updated/limits_updated · content: notes_saved/canvas_saved · team: member_added/member_removed/member_role_changed/ownership_transferred · snapshots: snapshot_captured/snapshot_deleted/snapshot_config/exported · serve: serve_started/serve_stopped · crash: crashed/crash_cleared) plus `isActivityAction`, `sanitizeDetails` (numbers/strings/string-arrays only, ≤400 serialized chars — booleans/objects/nested dropped), `appendActivity`/`capActivity` (injectable ceiling), `sliceActivity` (newest-first + honest total), `backfillFromLegacy` (kebab→snake `LEGACY_ACTION_MAP`, unknown skipped, order preserved) and `entryId()` (`a-<6>-<6>` base36)
